...
export default function AdminPaymentProvidersPage() {
  const result = useQuery({
    ...
    queryFn: () => adminApi.paymentProviders(),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          <p className="text-sm font-medium text-teal-300">Payments</p>
          <h1 className="mt-2 text-3xl font-semibold tracking-tight">Payment provider health</h1>
          <p className="mt-2 text-sm text-zinc-500">
            Checkout, subscription, invoice, and webhook provider status without exposing secrets.
          </p>
        </div>
        <button
